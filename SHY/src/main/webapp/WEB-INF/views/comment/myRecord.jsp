            ...
            <c:forEach var="com" items="${userCom}">
			              <tr class="myRecord_content">
			                    <td class="myRecord_content_nm">
			                     <fmt:formatDate value="${com.com_regdate}" pattern="yy/MM/dd"/>
			                  	</td> 
			                     <td class="myRecord_content_nm">
			                     	<c:choose>
						<c:when test="${com.com_type == 1}">
							<p>사기업게시판</p>
						</c:when>
						<c:when test="${com.com_type == 2}">
							<p>공기업게시판</p>
						</c:when>
						<c:when test="${com.com_type == 3}">
							<p>아시아게시판</p>
						</c:when>
						<c:when test="${com.com_type == 4}">
							<p>유럽게시판</p>
						</c:when>
						<c:when test="${com.com_type == 5}">
							<p>남미게시판</p>
						</c:when>
						<c:when test="${com.com_type == 6}">
							<p>북미게시판</p>
						</c:when>
						<c:when test="${com.com_type == 7}">
							<p>아프리카게시판</p>
						</c:when>
						<c:when test="${com.com_type == 8}">
							<p>국가자격증게시판</p>
						</c:when>
						<c:when test="${com.com_type == 9}">
							<p>민간자격증게시판</p>
						</c:when>
						<c:when test="${com.com_type == 10}">
							<p>어학게시판</p>
						</c:when>
						<c:when test="${com.com_type == 11}">
							<p>자유게시판</p>
						</c:when>
						<c:when test="${com.com_type == 12}">
							<p>취뽀게시판</p>
						</c:when>
						<c:when test="${com.com_type == 13}">
							<p>취업게시판</p>
						</c:when>
						<c:when test="${com.com_type == 14}">
							<p>자격증게시판</p>
						</c:when>
						<c:when test="${com.com_type == 15}">
							<p>자소서게시판</p>
						</c:when>
			                    ...
			                    <td class="myRecord_content_nm">${com.com_content}</td>
